...
func GenerateRepository(project schemas.Project) {
	createFileRepository(project)
	writeFileRepository(project)
}

func writeFileRepository(project schemas.Project) (string, error) {
	...
	filepath, err := filepath.Abs("./" + realName + "/domain/repository/" + realName + "_repository" + ".dart")
	if err != nil {
		log.Fatal("error")
		return filepath, err
	}
	...
	//Write some text line-by-line to file.
	_, err = file.WriteString("abstract class " + strings.Join(abstractName, "") + "Repository {\n")
	if isError(err) {
		return "", err
	}
...
func createFileRepository(project schemas.Project) (string, error) {
	var realFeatureName []string
	splitText := strings.Split(project.FeatureName, " ")
