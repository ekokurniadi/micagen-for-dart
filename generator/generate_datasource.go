...
func GenerateDataSource(project schemas.Project) {
...
func writeFileDataSource(project schemas.Project) (string, error) {
	...
	filepath, err := filepath.Abs("./" + realName + "/data/datasource/" + realName + "_datasource" + ".dart")
	if err != nil {
		log.Fatal("error")
		return filepath, err
	}
	...
	//Write some text line-by-line to file.
	_, err = file.WriteString("abstract class " + strings.Join(abstractName, "") + "DataSource {\n")
	if isError(err) {
		return "", err
	}
