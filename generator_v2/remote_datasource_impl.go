...
func GenerateRemoteDataSourceImpl(project schemas.Project) {
	createFileRemoteDataSourceImpl(project)
	writeFileRemoteDataSourceImpl(project)
}

func writeFileRemoteDataSourceImpl(project schemas.Project) (string, error) {
	...
	filepath, err := filepath.Abs("./features/" + realName + "/data/datasources/remote/" + realName + "_remote_datasource_impl" + ".dart")
	if err != nil {
		log.Fatal("error")
		return filepath, err
	}
...
func createFileRemoteDataSourceImpl(project schemas.Project) (string, error) {
	var realFeatureName []string
	splitText := strings.Split(project.FeatureName, " ")
