...
func GenerateLocalDataSource(project schemas.Project) {
	createFileDataSource(project)
	writeFileDataSource(project)
}

func writeFileDataSource(project schemas.Project) (string, error) {
	...
	filepath, err := filepath.Abs("./features/" + realName + "/data/datasources/local/" + realName + "_local_datasource" + ".dart")
	if err != nil {
		log.Fatal("error")
		return filepath, err
	}
	...
	//Write some text line-by-line to file.
	_, err = file.WriteString("abstract class " + strings.Join(abstractName, "") + "LocalDataSource {\n")
	if isError(err) {
		return "", err
	}
...
func createFileDataSource(project schemas.Project) (string, error) {
	var realFeatureName []string
	splitText := strings.Split(project.FeatureName, " ")
