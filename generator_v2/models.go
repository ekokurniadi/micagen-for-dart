...
func GenerateModels(entity schemas.Project) {
	createFileModels(entity)
	writeFileModels(entity)
}

func writeFileModels(entity schemas.Project) (string, error) {
	...
	filepath, err := filepath.Abs("./features/" + realName + "/data/models/" + strings.ToLower(entity.Entity.EntityName) + "_model.codegen" + ".dart")
	if err != nil {
		log.Fatal("error")
		return filepath, err
	}
	...
	if entity.Config.UseFreezed {
		_, err = file.WriteString("part \"" + strings.ToLower(entity.Entity.EntityName) + "_model.codegen.freezed" + ".dart\";\n")
		if isError(err) {
			return "", err
		}
		_, err = file.WriteString("part \"" + strings.ToLower(entity.Entity.EntityName) + "_model.codegen.g" + ".dart\";\n\n")
		if isError(err) {
			return "", err
		}
		...
		_, err = file.WriteString("class " + strings.Join(abstractName, "") + "Model with _$" + strings.Join(abstractName, "") + "Model{\n")
		if isError(err) {
			return "", err
		}
		_, err = file.WriteString("\nconst factory " + strings.Join(abstractName, "") + "Model({" + " \n")
		if isError(err) {
			return "", err
		}
		...
		_, err = file.WriteString("})=" + "_" + strings.Join(abstractName, "") + "Model;" + " \n\n")
		if isError(err) {
			return "", err
		}

		_, err = file.WriteString("\nfactory " + strings.Join(abstractName, "") + "Model.fromJson(Map<String,dynamic>json) => _$" + strings.Join(abstractName, "") + "ModelFromJson(json);\n\n")
		if isError(err) {
			return "", err
		}
		...
		//Write some text line-by-line to file.
		_, err = file.WriteString("class " + strings.Join(abstractName, "") + "Model extends Equatable{\n")
		if isError(err) {
			return "", err
		}
...
func createFileModels(entity schemas.Project) (string, error) {
	var realFeatureName []string
	splitText := strings.Split(entity.FeatureName, " ")
